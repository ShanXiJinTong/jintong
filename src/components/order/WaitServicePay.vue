	...
	<div style="padding-bottom:1.08rem;">
		...
		<div v-if="coinFlag" style="position: absolute;left:0;top:0;right:0;bottom:0;background: rgba(0,0,0,0.3);">
			<div style="position: fixed;z-index: 99999;bottom: 0;width: 100%;max-width: 7.5rem;height: 1.7rem;background: #f3f3f3;display: flex;align-items: center;justify-content: space-around;">
				<button style="width: 1.5rem;height: 0.7rem;outline: none;border: none;background: #E7F6FF;color: #3BACFE;border-radius:0.5rem;" @click="coin=0;coinFlag=false;">不使用
                </button>
				<button style="width: 1.5rem;height: 0.7rem;outline: none;border: none;background: #3BACFE;color: #fff;border-radius: 0.5rem;" @click="coin=coin1;coinFlag=false;">使用
                </button>
			</div>
		</div>
	...
	export default {
		...
		methods: {
          ...
          if(res.data.status ==1) {
            ...
            location.href = 'https://appserver.sxjtyb.cn/weixin/example/index.php?orderid='+res.data.order_id;
					}
				})
